...
/*   Updated: 2025/03/18 01:50:00 by jihyeki2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_token					*pars_state(t_pars *new, t_token *current,
							t_pars **pars_list);
...
int						check_error(char *command, char **arguments,
							int status);
int						oper_redir_in(t_pars *c, int stdin_save);
int						oper_redir_out(t_pars *c, int stdout_save);
int						oper_heredoc_in(int stdin_save, t_ori *ori,
							t_pipe *pipe, t_redir *save);
int						read_heredoc(int flag, t_ori *ori, t_pipe *pipe,
							t_redir *save);
...
void					free_child(t_ori *ori, int free_flag, t_pipe *pipe,
							t_redir *save);
void					free_child1(t_envp **lst, t_ori *ori, int free_flag,
							int files[]);
char					*get_next_line(int fd);

int						ft_pos_c(char *str, char c);
int						print_buff(char *buffer, int filenum);
int						put_heredoc(int fd, t_ori *ori, t_pipe *pipe,
	char name[]);
int						read_heredoc(int flag, t_ori *ori, t_pipe *pipe,
							t_redir *save);
...
void					process_heredoc(t_pars *command, t_redir **new_redir,
							t_redir **save_new_redir);
