...
/*   Updated: 2025/03/19 14:54:02 by jihyeki2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*int		add_arg_pipe(t_pars *pars_node, char *token_str)
{
	...
	if (!ret)
		return (ft_err_print_minishell("add arg pipe: ret"), ERR);
	i = 0;
	while (command)
	{
		ret[i] = ft_strdup(pars_node->command);
		ft_printf("ret : [%s]\n", ret[i]);
		if (!ret[i])
	...
	ret[i] = 0;
	free_pars_arguments(&pars_node->command);
	pars_node->command = *ret;
	pars_node->arguments = &ret[0];
	return (0);
}*/
...
int	keep_pars(t_pars *pars_node, t_token *token)
{
	...
	//else if (token->type == P)
		//return (add_arg_pipe(pars_node, token->str));
	return (0);
}

/*int	keep_pars(t_pars *pars_node, t_token *token) //origin
{
	if (token->type == W || token->type == DQ || token->type == SQ)
		return (add_arg(pars_node, token->str));
	else if (token->type == RI)
...
/*void	pars_state(t_pars *pars_node, t_token *token_curr, t_pars **pars_list) //origin
{
	while (token_curr && token_curr->type != P)
	{
		if (keep_pars(pars_node, token_curr))
